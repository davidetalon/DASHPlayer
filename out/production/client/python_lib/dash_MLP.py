...
#        global train_set, future_state_output, y, mask, i, action_vec
...
#        y = np.array([state_output[0,:]] * len(y_vec))
        ...
        _, loss = self.session.run([self.optimizer, self.loss], feed_dict={self.x: state_matrix, self.y: y, self.actions: action_vec, self.keep_prob: self.dropout_keep_prob})
